...
namespace csc232
{
    ...
    {
    public:
        CSC232BaseTestFixture( ) : buffer{ std::stringstream{ } }, sbuf{ nullptr }, earnedPoints{ 0 },
                                   possiblePoints{ 0 }
        {
    ...
    protected:
                                              ...
                                              const std::string & className, const std::string & methodName )
        {
            ...
            std::regex namespacePattern( "\\bnamespace\\s+" + namespaceName + "\\b" );
            std::regex classPattern( "\\bclass\\s+" + className + "\\b" );
            std::regex scopePattern( "\\bprivate\\b" );
            ...
            while ( std::getline( file, line ) )
            {
                ...
                if ( inClass && std::regex_search( line, scopePattern ) )
                {
                    inScope = true;
                }
                if ( inScope && std::regex_search( line, methodPattern ) )
                {
                    return true;
                }
            }
